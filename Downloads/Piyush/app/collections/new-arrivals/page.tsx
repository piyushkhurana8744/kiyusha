...
import { newArrivals as staticArrivals } from "@/data/home";
import { motion } from "framer-motion";
import { useEffect, useState } from "react";

export default function NewArrivalsPage() {
  const [products, setProducts] = useState<any[]>(staticArrivals);
    ...
    fetch("/api/products")
      ...
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) {
          const filtered = data.filter(p => p.isNewArrival || p.badge === "New");
      ...
      <header className="relative bg-warmWhite py-24 text-center overflow-hidden">
        ...
        >
          ...
          <p className="max-w-2xl mx-auto text-deepCharcoal/70 text-lg leading-relaxed font-light">
            Discover our latest collection of hand-finished jewelry, where modern silhouettes meet timeless craftsmanship. Sculpted for the contemporary woman.
          </p>
        </motion.div>
      </header>
      ...
      <section className="py-20 min-h-[60vh]">
        <div className="container-lux">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            ...
            {/* Filter/Sort Placeholder for aesthetic completeness */}
